...
TwoPoints::TwoPoints() :
  ...
  updateFactor(DEFAULT_UPDATE_FACTOR), model(0)
{
...
void TwoPoints::loadConfig()
{
  ...
  matchingThreshold = cvReadRealByName(fs, 0, "matchingThreshold", DEFAULT_MATCH_THRESH);
  updateFactor = cvReadRealByName(fs, 0, "updateFactor", DEFAULT_UPDATE_FACTOR);
