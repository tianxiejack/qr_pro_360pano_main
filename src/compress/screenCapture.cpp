...
static void formatlocation(FMT_mode mode)
{
	...
	if(mode == format_vid_open)
	{
		...
		localwrFd = fopen(filename_cur, "wb");
		localwrsize = 0;
		localwrframe = 0;
		if(localwrFd == NULL)
	...
	{
		sprintf(filename_cur, "%s/shot_%s.JPG", 
					video_dir, curtmstr);
		localwrFd = fopen(filename_cur, "wb");
		if(localwrFd == NULL)
...
static int outbufCb_vid(void* arg, unsigned char *buf, int size)
{
	...
	if(localwrFd != NULL)
	{
		ret = fwrite(buf, 1, size, localwrFd);
		if(ret != size)
		{
...
static GstPadProbeReturn fakesink_out_buffer (GstPad *pad, GstPadProbeInfo *info, gpointer user_data)
{
	CustomData *pData = (CustomData *)user_data;
	//gint iBufSize = 0;
	gchar* pbuffer = NULL;
...
static int gstlinkInit_appsrc_shot(CustomData* pData)
{
	...
	//pData->source = gst_element_factory_make ("appsrc", NULL);
	//pData->clockoverlay = gst_element_factory_make ("clockoverlay", "clockoverlay");
	pData->nvvidconv = gst_element_factory_make ("videoconvert", NULL);
	...
	if (!pData->pipeline || !pData->source || !pData->jpegenc || !pData->fakesink1)
	{
		g_printerr ("Not all elements could be created.\n");
		return -1;
	}
	...
	gst_bin_add_many (GST_BIN(pData->pipeline), pData->source, /*pData->clockoverlay, *//*pData->videoconvert, */pData->nvvidconv, pData->jpegenc, pData->fakesink1, NULL);
	...
	pData->caps_enc_to_jpeg = gst_caps_new_simple("image/jpeg",
								...
								"framerate", GST_TYPE_FRACTION, pData->framerate, 
								1, NULL);
	if(!gst_element_link_filtered(pData->jpegenc, pData->fakesink1, pData->caps_enc_to_jpeg))
	{
	...
	//pData->bPushPause = true;		// shot start stat is pause
	//pthread_create(&pData->threadPushBuffer, NULL, thrdhndl_push_buffer, (void*)pData);
	//OSA_assert(pData->threadPushBuffer != 0);
...
static int screenInit(CustomData* pDataVid, CustomData* pDataShot)
{
	...
	if(res == -1)
		return -1;
	res = gstlinkInit_appsrc_shot(pDataShot);
	if(res == -1)
		return -1;
/*
	pthread_t thread_0;
...
int gst_videnc_create(void)
{
	...
	screenInit(&customData[0], &customData[1]);
	return 0;
}

int gst_videnc_enable(int bEnable)
{
	customData[0].bRec = (bEnable)?true:false;
...
{
	customData[1].bRec = true;
	customData[1].bPushPause = false;
